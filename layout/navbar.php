...
<nav class="bg-white border-b border-gray-200 sticky top-0 z-50 backdrop-blur-sm bg-white/95">
        ...
        <div class="flex items-center justify-between h-16">
            ...
            <div class="flex items-center gap-3">
                <a href="<?= BASE_URL ?>/index.php" class="flex items-center gap-2.5 group">
                    ...
                    <span class="text-lg font-semibold text-gray-800 hidden sm:block">MedWeb</span>
                </a>
            </div>
                ...
                <div class="flex items-center gap-3 pl-3 border-l border-gray-200">
                    ...
                    <div class="relative" id="userDropdown">
                        <button onclick="toggleDropdown('userDropdownMenu')" 
                                class="flex items-center gap-2.5 px-2 py-1.5 rounded-lg hover:bg-gray-50 focus-ring transition-colors">
                            ...
                            <div class="text-left">
                                <p class="text-sm font-medium text-gray-700 leading-none"><?= htmlspecialchars($currentUser['name'] ?? '') ?></p>
                                <p class="text-xs text-gray-400 mt-0.5"><?= htmlspecialchars($currentUser['email'] ?? '') ?></p>
                            </div>
                        ...
                        <div id="userDropdownMenu" 
                             class="hidden absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-gray-200 py-1.5 z-50 animate-in">
                            <div class="px-4 py-2.5 border-b border-gray-100">
                                <p class="text-sm font-medium text-gray-700"><?= htmlspecialchars($currentUser['name'] ?? '') ?></p>
                                <p class="text-xs text-gray-400"><?= htmlspecialchars($currentUser['email'] ?? '') ?></p>
                            </div>
                            <a href="<?= BASE_URL ?>/auth/logout.php" class="flex items-center gap-2 px-4 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors">
            ...
            <div class="md:hidden">
                <button onclick="toggleMobileMenu()" 
                        class="p-2 rounded-lg text-gray-500 hover:text-gray-700 hover:bg-gray-100 focus-ring transition-colors">
    ...
    <div id="mobileMenu" class="hidden md:hidden border-t border-gray-100">
        <div class="px-4 py-3 space-y-1">
            <a href="<?= BASE_URL ?>/index.php" class="block px-3 py-2 text-sm font-medium text-gray-600 hover:text-cyan-600 hover:bg-cyan-50 rounded-lg transition-colors">
                Dashboard
            </a>
        </div>
        <div class="border-t border-gray-100 px-4 py-3">
            ...
            <a href="<?= BASE_URL ?>/auth/logout.php" class="flex items-center gap-2 px-3 py-2 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/>
                </svg>
                Sign out
